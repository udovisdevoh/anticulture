...
{
    public class Tell : Operation
    {
        #region Static operator
        public static readonly Operator Operator = new Operator("tell", Factory, 1);

        private static Operation Factory(Human who, Entity[] what)
        {
            if (what.Length != 1) throw new ArgumentException("Tell takes a single argument");
            ...
            return new Tell(who, what[0] as Human, bestNeed, bestSolution);
        }
        #endregion
        ...
        #region Constructor
        public Tell(Human who, Human what, Human.Need bestNeed, Human.Need.Solution bestSolution)
        {
            mWho = who;
            mWhat = what;
            mBestNeed = bestNeed;
        ...
        public override void Update(Timer timer, Random random)
        {
            ...
            if (mTimeLeft <= 0.0f)
            {
                ...
                if (!found)
                    need.Solutions.Add(new Human.Need.Solution(mBestSolution.Prototype, mBestSolution.Effectiveness));
            }
        }
